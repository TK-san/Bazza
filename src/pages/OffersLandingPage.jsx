/**
 * OffersLandingPage
 * Landing page showing service categories for Offers tab
 */
...
import { categories, mockOffers } from '../data/mockServices';
...
const OffersLandingPage = ({ onSelectCategory }) => {
  const { t } = useLanguage();

  // Filter out 'all' category and count offers per category
  const categoryList = categories.filter((cat) => cat.id !== 'all');

  const getCategoryCount = (categoryId) => {
    return mockOffers.filter((offer) => offer.category === categoryId).length;
  };
    ...
    >
      ...
      <Box px={4} pt={6} pb={4} bg="white">
        ...
        <Text fontSize="sm" color="gray.500">
          {t('offers.subtitle')} - {totalOffers} {t('offers.services')}
        </Text>
      </Box>
        ...
        >
          <VStack gap={1}>
            <Text
              ...
              color="white"
            >
              {t('categories.all')} {t('offers.services')}
            </Text>
            <Text fontSize="sm" color="blue.100">
              {totalOffers} {t('offers.services')}
            </Text>
          </VStack>
        </Box>
      </Box>
